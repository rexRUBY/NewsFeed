...
public class FollowerService {
    ...
    // post - 친구 팔로우
    @Transactional
    public FollowerSaveResponseDto saveFollower(AuthUser authUser, FollowerSaveRequestDto followerSaveRequestDto) {
        // 사용자의 id가 존재하는지 체크
        User user = userRepository.findById(authUser.getId()).orElseThrow(()-> new NullPointerException("사용자가 존재하지 않습니다."));

        // 추가하려는 타 유저의 id가 존재하는지 체크 -> 없다면 추가
        User follower = followerRepository.findByEmail(followerSaveRequestDto.getEmail()).orElseThrow(()-> new NullPointerException("사용자가 존재하지 않습니다."));
...
//    public FollowAcceptResponseDto acceptFollow(String email) {
//
//    }
